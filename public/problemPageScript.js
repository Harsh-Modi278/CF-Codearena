...
socket.on("compete-message", (handles, userClasses) => {
  ...
  userTitles[0].innerHTML =
    " <strong class =" +
    userClasses[handles[1 - otherIndx]] +
    ">" +
    username +
    "</strong>";
  userTitles[1].innerHTML =
    " <strong class =" +
    userClasses[handles[otherIndx]] +
    ">" +
    handles[otherIndx] +
    "</strong>";
...
function onsubmit() {
  ...
  var link = ele.href;
  link = link.split("/");
  var contestId = parseInt(link[4]),
    problem_index = link[6];
  const url =
  ...
  fetch(url)
    ...
    .then((data) => {
      if (data.status === "OK") {
        ...
        if (
          obj.problem_index !== problem_index ||
...
socket.on("display-logs", ({ handle, obj }) => {
  ...
  message.innerHTML =
    "Verdict for Last Submission is " +
    "<b>" +
    obj.verdict +
    "</b>" +
    " on <i>" +
  ...
  for (let i = 0; i < logWin.length; i++)
    logWin[i].scrollTop = logWin[i].scrollHeight;
  if (obj.verdict === "OK") {
    if (username === handle) {
